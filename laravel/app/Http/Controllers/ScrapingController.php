...
class ScrapingController extends Controller
{
  public function signin(Request $request)
  {
    ...
    try {
      ...
      return response(['status' => 401, 'message' => 'unauthorized error'], 401);
    }
  }
  ...
  public function news(Request $request)
  {
    ...
    // データの取得
    $crawler->filter('ul.news_list01 li')->each(function ($node) use (&$links, &$titles, &$tags, &$dates, &$ids) {
      $titles[] = $node->filter('dd')->text(); // タイトル
      $dates[] = strstr($node->filter('dt')->text(), ' ', true); // date 空白から左
      $tags[] = $node->filter('span')->text(); // タグ
      $links[] = (string)env('APP_NEWS_LINK') . strstr($node->filter('a')->attr('href'), '?'); // リンク
  ...
  public function only(Request $request, $id)
  {
    ...
    // サイトデータ取得
    $crawler = $client->request('GET', (string)env('APP_NEWS_FRONT') . $id . (string)env('APP_NEWS_REAR'));
    ...
    foreach($attachments as $i => $attachment){
      $attachments[$i] = (string)env('APP_DOMAIN') . substr(mb_strstr(substr($attachment, 7), 'class', true), 0, -2);
    }
  ...
  public function calendar($year, $month, Request $request)
  {
    ...
    $crawler->filter('ul.calendar_list01 li')->each(function ($node) use (&$days, &$plans) {
      ...
      foreach($links as $i => $link){
        $links[$i] = str_replace('amp;', '', (string)env('APP_CALENDAR') . substr(mb_strstr(substr($link, 12), 'style', true), 0, -2));
      }
  ...
  public function attendance(Request $request)
  {
    ...
    $token = strstr(strstr($client->getHistory()->current()->getUri(), '('), ')', true).'))';
    ...
    // サイトデータ取得
    $crawler = $client->request('GET', (string)env('FALCON_FRONT') . $token . (string)env('FALCON_ATTENDANCE'));
  ...
  public function timetable(Request $request)
  {
    ...
    // サイトデータ取得
    $crawler = $client->request('GET', (string)env('FALCON_FRONT') . $token . (string)env('FALCON_TIMETABLE'));
    ...
    foreach($dates as $i => $date){
      ...
      $datas = array_filter(str_replace('限:', '', preg_split('/[0-9]/', preg_replace("/( |　)/", "", str_replace('行事 ', '', strstr(strstr($client->click($crawler->selectLink($date)->link())->text(), '行事'), '戻る', true))))));
